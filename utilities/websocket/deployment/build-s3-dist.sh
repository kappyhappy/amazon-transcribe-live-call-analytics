...
cd ..
zip -r out/lca-websocket.zip ./ -x ./node_modules/**\*
cd deployment

# git ls-files .. | xargs zip -@ --filesync ${OUT_DIR}/audiohooksrc.zip
export RELEASE_S3_BUCKET=${RELEASE_S3_BUCKET_BASE}-${AWS_REGION}
aws s3 cp ${OUT_DIR}/lca-websocket.zip s3://${RELEASE_S3_BUCKET}/${RELEASE_S3_PREFIX}/lca-websocket.zip

TEMPLATE_FILE="./lca-websocket.yaml"
RELEASE_S3_PREFIX_SUB=${RELEASE_S3_PREFIX////_}
PACKAGE_RELEASE_REPLACE_OUT_FILE=${OUT_DIR}/template-replaced-${RELEASE_S3_BUCKET}-${RELEASE_S3_PREFIX_SUB}.yaml
sed -E \
		" \
...
PACKAGE_RELEASE_FILE_NAME=template-packaged-${RELEASE_S3_BUCKET}-${RELEASE_S3_PREFIX_SUB}.yaml
PACKAGE_RELEASE_OUT_FILE=${OUT_DIR}/${PACKAGE_RELEASE_FILE_NAME}

echo "[INFO] sam building template file for release"
sam build \
	--use-container \
	--parallel \
	--cached \
...
echo "[INFO] sam packaging for release"
sam package \
	--s3-bucket ${RELEASE_S3_BUCKET} \
	--s3-prefix ${RELEASE_S3_PREFIX} \
	--output-template-file ${PACKAGE_RELEASE_OUT_FILE} 

RELEASE_TEMPLATE_S3_URL=s3://${RELEASE_S3_BUCKET}/${RELEASE_S3_PREFIX}/lca-websocket.yaml
...
echo "[INFO] CloudFormation template URL: https://${RELEASE_S3_BUCKET}.s3.amazonaws.com/${RELEASE_S3_PREFIX}/lca-websocket.yaml"